...
public class BeautifulToast extends LinearLayout {
    ...
    public static BeautifulToast makeText(@NonNull Context context, String text, int length, @StyleRes int style) {
        return new BeautifulToast(context, text, length, style);
    }

    public static BeautifulToast makeText(@NonNull Context context, String text, @StyleRes int style) {
        return new BeautifulToast(context, text, Toast.LENGTH_SHORT, style);
    }

    private BeautifulToast(@NonNull Context context, String text, int length, @StyleRes int style) {
        super(context);
        this.text = text;
        this.length = length;
        this.style = style;
    ...
    private BeautifulToast(Builder builder) {
    ...
    private void inflateToastLayout() {
        View v = inflate(getContext(), R.layout.beautiful_toast_layout, null);
        rootLayout = (LinearLayout) v.getRootView();
        textView = v.findViewById(R.id.textview);
        if (style > 0) {
            typedArray = getContext().obtainStyledAttributes(style, R.styleable.BeautifulToast);
        }
    ...
    private void loadShapeAttributes() {
        ...
        solidBackground = typedArray.getBoolean(R.styleable.BeautifulToast_stSolidBackground, false);
        backgroundColor = typedArray.getColor(R.styleable.BeautifulToast_stColorBackground, defaultBackgroundColor);
        cornerRadius = (int) typedArray.getDimension(R.styleable.BeautifulToast_stRadius, defaultCornerRadius);
        length = typedArray.getInt(R.styleable.BeautifulToast_stLength, 0);
        gravity = typedArray.getInt(R.styleable.BeautifulToast_stGravity, Gravity.BOTTOM);
        ...
        if (typedArray.hasValue(R.styleable.BeautifulToast_stStrokeColor) && typedArray.hasValue(R.styleable.BeautifulToast_stStrokeWidth)) {
            strokeWidth = (int) typedArray.getDimension(R.styleable.BeautifulToast_stStrokeWidth, 0);
            strokeColor = typedArray.getColor(R.styleable.BeautifulToast_stStrokeColor, Color.TRANSPARENT);
        }
    }
    ...
    private void loadTextViewAttributes() {
        ...
        textColor = typedArray.getColor(R.styleable.BeautifulToast_stTextColor, textView.getCurrentTextColor());
        textBold = typedArray.getBoolean(R.styleable.BeautifulToast_stTextBold, false);
        textSize = typedArray.getDimension(R.styleable.BeautifulToast_stTextSize, 0);
        font = typedArray.getResourceId(R.styleable.BeautifulToast_stFont, 0);
    ...
    private void loadIconAttributes() {
        ...
        iconStart = typedArray.getResourceId(R.styleable.BeautifulToast_stIconStart, 0);
        iconEnd = typedArray.getResourceId(R.styleable.BeautifulToast_stIconEnd, 0);
    }
    ...
    public static class Builder {
        ...
        /**
         * This call will make the BeautifulToast's background completely solid without any opacity.
         */
        ...
        /**
         * @param cornerRadius Sets the corner radius of the BeautifulToast's shape.
         */
        ...
        /**
         * Sets where the BeautifulToast will appear on the screen
         */
